"""
팩트버스 (FactVerse) - Streamlit 리워드 페이지 (v5)

이번 변경점:
- 기프티콘 메뉴를 GS25 바나나맛우유 / GS25 가나초콜릿마일드 / 배스킨라빈스 싱글레귤러 / 공차 블랙밀크티(라지)로 변경
  (2026년 7월 기준 실제 매장/편의점 판매가와 동일한 포인트로 설정)
- 사이드바 누적 포인트 실시간 반영 버그 재점검(매 스크립트 실행마다 지갑을 디스크에서 다시 불러옴)
- 팀원이 만든 ZEP 메타버스 팩트버스 안에서 "웹사이트 열기" 오브젝트로 이 앱을 연결하는 용도의
  독립형 퀴즈+포인트+기프티콘 교환 리워드 페이지 (ZEP 내장 OX퀴즈와는 별도로 동작함)

실행 방법:
    1) pip install streamlit
    2) streamlit run factverse_streamlit_app_v5.py

배포 시 ZEP 오브젝트 링크에는 "https://앱주소.streamlit.app/?embed=true" 형태로 넣으면
스트림릿 자체 UI(메뉴/푸터)가 숨겨져 팝업 안에 더 깔끔하게 표시됨.
"""
...
# ---------- 기프티콘: 실제 매장/편의점가와 동일한 포인트로 설정 (2026년 7월 확인) ----------
GIFTICONS = [
    {"name": "GS25 바나나맛우유(빙그레)", "cost": 1400},
    {"name": "GS25 가나초콜릿마일드", "cost": 1000},
    {"name": "배스킨라빈스 싱글레귤러", "cost": 3900},
    {"name": "공차 블랙밀크티(라지)", "cost": 4000},
]
